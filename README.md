# 🎯 Video Labeling Tool with AI (YOLOv8 + DINOv2)

동영상에서 객체를 수동으로 라벨링하고, YOLOv8과 DINOv2를 활용한 AI 자동 라벨링 기능을 제공하는 웹 기반 도구입니다.

## ✨ 주요 기능

- 🎥 **웹 기반 비디오 라벨링**: 브라우저에서 직접 동영상을 업로드하고 라벨링
- 🖱️ **직관적인 인터페이스**: 마우스 드래그로 간편한 바운딩 박스 생성
- 🤖 **AI 자동 라벨링**: YOLOv8 + DINOv2를 활용한 정밀한 객체 탐지
...
```
Frontend (웹 브라우저)          Backend (Python 서버)
┌─────────────────────┐       ┌─────────────────────┐
...
- **사용 모델**: `yolov8n.pt` (nano 버전, 빠른 추론)
...
#### **통합 워크플로우**
1. **YOLO 탐지**: 기본 객체 탐지 (person, car 등)
...
#### **🧠 DINOv2 지능형 기능**
- `POST /api/learn_dinov2_patterns`: 수동 라벨링에서 DINOv2 패턴 학습
- `GET /api/get_dinov2_patterns_info`: 학습된 패턴 정보 조회
...
2. **🚀 향후 개발 계획**:
   ...
   - **실시간 처리**: 웹캠 실시간 라벨링 + DINOv2 분류
   - **데이터베이스 연동**: 학습된 패턴 영구 저장 및 공유
   - **다중 사용자**: 협업 라벨링 + 패턴 공유 기능
   - **고급 모델**: Transformer 기반 객체 탐지 모델 통합
...
1. **"서버에 연결할 수 없습니다"**
   ...
   - `python run_server.py` 실행

2. **"AI 모델 로딩 중..."**
   - 첫 실행시 모델 다운로드로 시간 소요
   - 인터넷 연결 및 충분한 저장공간 확인
